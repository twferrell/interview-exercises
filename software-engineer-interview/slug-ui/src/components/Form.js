  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="form-body">
        ...
        <input type="text" {...useInput('description', 'isRequired')} />
      </div>
      ...
      {resp && resp.complete && !resp.error && (
        <a className="slug-url" href={resp.data.url}>
          Slug: {resp.data.id}
        </a>
      )}
      ...
      {resp && resp.complete && resp.error && (
        <span className="error-message">
          Sorry, an error occurred while trying to create your slug!
        </span>
      )}
    </form>
  );
